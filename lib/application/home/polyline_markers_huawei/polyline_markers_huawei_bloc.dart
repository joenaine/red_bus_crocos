...
part 'polyline_markers_huawei_event.dart';
part 'polyline_markers_huawei_state.dart';
part 'polyline_markers_huawei_bloc.freezed.dart';
...
class PolylineMarkersHuaweiBloc
    extends Bloc<PolylineMarkersHuaweiEvent, PolylineMarkersHuaweiState> {
  PolylineMarkersHuaweiBloc() : super(PolylineMarkersHuaweiState.initial()) {
    on<PolylineMarkersHuaweiEvent>((event, emit) async {
      await event.map(
  ...
  void _saveData(List<LatLng> latLngList) {
    final List<LatLngData> dataList =
        latLngList.map((latLng) => LatLngData(latLng.lat, latLng.lng)).toList();
  ...
  Future<List<LatLng>> getPolylinePoints() async {
    ...
    for (int i = 0; i < polylineList.length - 1; i++) {
      ...
      PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
          GOOGLE_MAPS_API_KEY,
          PointLatLng(startPoint.lat, startPoint.lng),
          PointLatLng(endPoint.lat, endPoint.lng),
